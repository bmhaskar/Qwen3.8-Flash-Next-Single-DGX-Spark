...
docker run \
    ...
    --served-model-name qwen3.8-flash-next --tensor-parallel-size 1 --gpu-memory-utilization 0.780 --max-num-seqs 4 --max-num-batched-tokens 2048 --max-model-len 262144 --kv-cache-dtype fp8 --load-format safetensors --safetensors-load-strategy lazy --enable-chunked-prefill --reasoning-parser qwen3 --enable-auto-tool-choice --tool-call-parser qwen3_coder --distributed-executor-backend mp --speculative-config '{"method":"mtp","num_speculative_tokens":3}' --compilation-config '{"mode":0,"cudagraph_mode":"FULL_DECODE_ONLY","cudagraph_capture_sizes":[4,8,12,16]}' --default-chat-template-kwargs '{"enable_thinking":false}' \
    --host 0.0.0.0 \
    --port 8888
