...
> ## This is a fork. All of the recipe below is Mia's work.
>
> Upstream: **[MiaAI-Lab/Qwen3.8-Flash-Next-Single-DGX-Spark](https://github.com/MiaAI-Lab/Qwen3.8-Flash-Next-Single-DGX-Spark)**
> by [Mia's AI Lab](https://x.com/MiaAI_lab) ([sponsor](https://github.com/sponsors/MiaAI-Lab)) ·
> checkpoint [`Mia-AiLab/Qwen3.8-Flash-Next-NVFP4`](https://huggingface.co/Mia-AiLab/Qwen3.8-Flash-Next-NVFP4) ·
> base model [Qwen3.8-Flash-Next](https://huggingface.co/Qwen/Qwen3.8-Flash-Next) by Qwen / Alibaba.
> Full attribution for the parts this is built from is in [Credits](#credits); it is unchanged.
>
> **Not one line of the recipe is modified.** `start.sh`, `stop.sh`, `download.sh`
> and every patch generator in `files/` are byte-identical to upstream `78b0675`.
> This fork exists only to record *which configuration is actually serving* on one
> host, `spark-bharat` — and, because the upstream project is AGPL-3.0-or-later and
> this runs as a network server, to publish that configuration rather than keep it
> private. See [RUNNING.md](RUNNING.md) for the operational detail.

## Fork changelog — what changed, and the issue behind each change

Three files, no code. Each entry is the observed problem first, then what was done
about it.

**1. `.env` and `.last_launch.sh` are now tracked.**
Upstream's `.gitignore` is an allowlist that deliberately excludes both. That is the
right call for a general recipe and it makes one thing impossible: reproducing a
*specific* known-good server. The tracked tree of a fresh clone is identical whether
the box is serving well or not at all. Re-included, together with `RUNNING.md`.

**2. This host launches from `.last_launch.sh`, not `./start.sh`.**
`.last_launch.sh` here was generated on upstream `ef1af5f`; the tree sits on
`78b0675`, and `start.sh` changed by **139 lines** in between — the `ABLIT=0/1`
gated-checkpoint feature, and checkpoint resolution moving from alphabetical to
state-based. Neither is tested on this box. The launch script is the literal
`docker run` behind a server that was measured good, so it bypasses the drift.
This is a local caution about *our* upgrade timing, not a defect in `start.sh`.
Relatedly, `.env` here predates the current `.env.sample` and is missing `ABLIT`,
`EXTRA_DOCKER_ARGS` and `MAMBA_SSM_CACHE_DTYPE`; reconciling the two is a
deliberate, measured relaunch, not a passing edit.

### Observed on this host — divergences from the README's own numbers

These are single-host observations, not corrections to upstream. They are recorded
because they cost time to rediscover.

- **KV pool came out at 389,398 tokens** (1.49x a 262,144-token request), against
  the 992,584 tokens the [Measured profile](#measured-profile) table reports for
  262k / FP8. Reproducible across relaunches here (391,943 on a previous one).
...
- **`--default-chat-template-kwargs '{"enable_thinking":false}'` is required** by
  ...
  opts back in per-request.

### Two GB10 host hazards worth knowing before you relaunch

Neither is caused by this recipe; both will waste an evening.

- **Unified memory leaks on any large-CUDA teardown** — all of it, on a clean exit,
  not just on `kill -9`. After a stop, `MemFree` was 13.5 GiB out of ~96 GB held.
  vLLM needs ~100 GiB of free *physical* memory to load, so a reclaim cycle
  (stop `gdm`, free memory, restart `gdm`, **regenerate the CDI spec**) is mandatory
  before relaunching. Skipping the CDI regen produces a misleading
  `CUDA unknown error` *inside* the container while the host's
  `torch.cuda.is_available()` is still True. Steps are in [RUNNING.md](RUNNING.md).
- **Run long operations under `tmux`.** This host's SSH link drops silently. A
  `stop.sh` was once killed mid-flight *after* it archived a still-running
  container's log but *before* its `docker rm -f` — one step from force-removing a
  healthy server and paying the full reload above.
