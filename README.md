...
**2. The launch script carried a live Hugging Face token.**
`.last_launch.sh`, as generated by `./start.sh --no-launch`, inlines the resolved
value as `-e HF_TOKEN=hf_…`. That is harmless while the file stays untracked and
becomes a leak the moment it isn't. Scrubbed to `-e HF_TOKEN=${HF_TOKEN}`, with
`HF_TOKEN` left commented out in `.env`. `HF_HUB_OFFLINE=1` is set, so serving an
already-downloaded checkpoint needs no token at all. **If you adopt this pattern,
scrub before the first commit** — a token in git history outlives the file.

**3. This host launches from `.last_launch.sh`, not `./start.sh`.**
